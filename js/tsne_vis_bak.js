...
function init(data, tabletop) {
    ...
    dists = computeDistances(data, document.getElementById("distance").value, document.getElementById("transform").value);
...
function computeDistances(data, distFunc, transFunc) {
  ...
  // function that changes the transform and restarts t-SNE
  function setTransformOrDistance() {
  ...
  function drawEmbedding() {
    ...
    d3.select("#perplexity_slider").call(
      ...
      // Rerun t-SNE if we change transform
      d3.select("#transform").on("change", function() {
	  setTransformOrDistance(); 
      });
      // Rerun t-SNE if we change distance metric
