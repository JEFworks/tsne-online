// tsne for count based processes
...
(function(global) {
  ...
  var xtod = function(X) {
    ...
    for(var i=0;i<N;i++) {
      for(var j=i+1;j<N;j++) {
          //var d = Math.sqrt(L2(Math.log(X[i]+1), Math.log(X[j]+1))); // MOD: root, log
          var d = L2(Math.log(X[i]+1), Math.log(X[j]+1)); // MOD: log
        dist[i*N+j] = d;
        dist[j*N+i] = d;
      }
    }
  ...
  var d2p = function(D, perplexity, tol) {
    ...
    for(var i=0;i<N;i++) {
	  ...
	  //var pj = 1/(Math.exp(D[i*N+j]) + Math.exp(-D[i*N+j]) + 2); // MOD: logistic
